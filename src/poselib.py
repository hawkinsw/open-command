...
What lives here:
  ...
  candidate_runs:            contiguous ball-detection runs whose timing is
                             ...
                             RELEASE_HALF_S), so warm-ups/replays can't
...
RELEASE_HALF_S = 0.60   # run-selection and t0 band around a known release
...
def candidate_runs(df, fps, T_plate):
    ...
    lo, hi = ((RELEASE_LO_S, RELEASE_HI_S) if np.isnan(rel)
              else (rel - RELEASE_HALF_S, rel + RELEASE_HALF_S))
